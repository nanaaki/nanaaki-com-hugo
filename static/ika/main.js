var app = new Vue({
  ...
  data: {
    ...
    gear: [
            ...
            {code:'W', name:'復活ペナルティ',
             calc: undefined },
            {code:'X', name:'ステルスジャンプ',
             calc: undefined },
            {code:'Y', name:'対物攻撃力アップ',
  ...
  computed: {
    ...
    styles: function (){
      var ink_ika = this.getStatus("3").ika/60+"s";
      var ink_hito = this.getStatus("3").hito/60+"s";
      var buki_ink = this.getStatus("1");
      var move_hito = (3*(50/this.getStatus("5").middle)/60)+"s";
      var move_ika = (3*(50/this.getStatus("6").middle)/60)+"s";
      ...
      return {
        ...
        '--move-hito' : move_hito,
        '--move-ika' : move_ika,
        '--move-aite' : (50/this.getStatus("D").hito)/60*3+"S",
        '--slip-damege-cap' : 150*(this.getStatus("D").cap/100)+"px",
        '--slip-damege-sec' : (this.getStatus("D").cap/this.getStatus("D").slip)/60+"s",
